...
router.post('/verify-crypto', protect, async (req, res) => {
  try {
    ...
    console.log(`Verifying Crypto TX: ${txHash} for Order: ${orderId} (${amountPaid} ${currency})`);
    ...
    if (order.paymentMethod !== 'wallet') {
      return res.status(400).json({ success: false, message: 'Order not configured for crypto payment' });
    }
    ...
    */
    ...
    res.json({ success: true, message: 'Crypto payment verified and order confirmed' });

  } catch (error) {
    console.error('Crypto Verification Error:', error.message);
    res.status(500).json({ success: false, message: error.message || 'Error processing crypto verification' });
  }
});
// Create Razorpay order
router.post('/create-order', protect, async (req, res) => {
